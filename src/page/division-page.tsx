...
import { DivisionProblemGenerator, DivisionProblem } from '../models/division';
...
const generator = new DivisionProblemGenerator(10, 0);
const getResults = (set: DivisionProblem[]) => {
    let i = 0;
    const results = [];
    while (i < set.length) {
...
const QuickDivisionPage: FC<object> = ({ }) => {
    useEffect(() => {
        ReactGA.pageview("/division");
    }, []);
    const [results, setResults] = useState<ProblemResult<DivisionProblem>[]>(getResults(generator.getProblems(10)));
    ...
    return (<>
        <ProblemHeader title="Quick Division" />
        {!showResults && <>
            <ProblemControl
                opperand="&divide;"
...
export default QuickDivisionPage;